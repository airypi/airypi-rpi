...
class SocketIO(object):
    ...
    def __init__(
        ...
        self._server_namepsace = server_namespace
    ...
    def _transport(self):
        ...
        for elapsed_time in warning_screen:
            try:
                ...
                #define namespace for path
                for path in self._namespace_by_path:
                    if len(path) > 0:
                        self.__transport.connect(path)
                        self._namespace_by_path[path].path = path
                break
            except ConnectionError as e:
